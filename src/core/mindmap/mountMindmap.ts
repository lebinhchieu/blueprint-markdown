...
export function mountMindmap(
  ...
  const cy = cytoscape({
    ...
    style: [
      ...
      },
      {
        ...
        // `outline-*` (not `border-*`): border-width feeds into outerWidth/
        // outerHeight, which edge endpoints anchor to — toggling it shifts
        // where connected edges/arrows terminate. Outline draws outside the
        // shape without affecting those dims, so edges never shift.
      ...
      {
        // The node whose content is currently shown in the drawer.
        selector: '.em-mindmap-active',
        style: {
          'outline-width': 3,
          'outline-style': 'solid',
  ...
  function renderDrawerContent(node: MindmapNode): void {
    // A cross-link click reaches here by replacing drawer.body mid-hover, so the
    // `<a>` that had the pointer on it is destroyed before its mouseleave can
    // fire — clear any highlight left over from that rather than leaving it stuck.
    cy.$('.em-mindmap-link-hover').removeClass('em-mindmap-link-hover')
    if (activeNodeId) cy.$id(activeNodeId).removeClass('em-mindmap-active')
    activeNodeId = node.id
    cy.$id(node.id).addClass('em-mindmap-active')
  ...
  function closeDrawer(): void {
    slideClosed(() => {
      setSideIfChanged('right')
      if (activeNodeId) {
        cy.$id(activeNodeId).removeClass('em-mindmap-active')
